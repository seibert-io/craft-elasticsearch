...
class Index extends Component
{
                    ...
                    'must'   => [
                        [
                            'multi_match' => [
                                ...
                                'analyzer' => $analyzer,
                                'operator' => 'and',
                                'fields'   => ['title^6', 'title.edge_ngram^2', 'description', 'description.edge_ngram',  'attachments.parsed.content', 'attachments.parsed.content.edge_ngram'/*, 'title.ngram^0.1', 'description.ngram^0.1', 'attachments.parsed.content.ngram^0.1'*/],
                            ]
                        ]
                    ],
                    'filter' => [
        ...
        return [
            'query' => [
                'multi_match' => [
                    ...
                    'analyzer' => $analyzer,
                    'operator' => 'and',
                    'fields'   => ['title^6', 'title.edge_ngram^2', 'description^2', /*'description.edge_ngram^2',*/ 'attachments.parsed.content', 'attachments.parsed.content.edge_ngram', /* 'title.ngram^0.1', 'description.ngram^0.1', 'attachments.parsed.content.ngram^0.1'*/],
                ]
            ],
            'suggest' => [
                'phrases' => [
                    ...
                    "aggregations" => [
                        "keywords" => [
                            "significant_text" => [
                                ...
                                "source_fields" => ["attachments.parsed.content", "title", "description"]
                            ]
                        ]
                    ]
                ]
    ...
    {
        $event->params['pipeline'] = 'attachments';
    }

    public function onBeforeCreateIndex(): void
    {
        try {
            ElasticSearchPlugin::$plugin->client->get()->ingest()->getPipeline(['id' => 'attachments']);
        } catch (Missing404Exception $e) {
            Craft::info('Creating pipeline \'attachments\'');

            ElasticSearchPlugin::$plugin->client->get()->ingest()->putPipeline([
                'id' => 'attachments',
                'body' => [
                    'description' => 'attachments ingest processor',
                    'processors' => [
                        [
                            "foreach" => [
                                'field' => 'attachments',
                                'processor' => [
                                    'attachment' => [
                                        ...
                                        'target_field' => '_ingest._value.parsed',
                                        'indexed_chars' => -1,
                                        'ignore_missing' => true,
                                    ]
                                ]
